    ...
    <div className="min-h-screen p-4 md:p-6 pb-24">
      <header className="mb-6">
        <div className="flex items-center gap-3 mb-4">
          <SearchIcon className="w-8 h-8 text-bionic-accent" />
          <h1 className="text-2xl font-bold text-bionic-text">Search</h1>
        </div>
        <input
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder="Search users, posts, goals..."
          className="w-full px-4 py-3 rounded-xl glass-input text-bionic-text"
          autoFocus
        />
        <div className="flex gap-2 mt-3">
          {(["all", "users", "posts"] as const).map((t) => (
            <button key={t} onClick={() => setType(t)} className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${type === t ? "bg-bionic-accent text-bionic-bg" : "glass-button text-bionic-text-dim"}`}>
              ...
              <div className="space-y-2">
                {results.posts.map((p) => (
                  <div key={p.id} className="glass-card p-3">
                    ...
                    {p.image_url && <img src={p.image_url} alt="" className="mt-2 rounded-lg max-h-32 w-full object-cover" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />}
